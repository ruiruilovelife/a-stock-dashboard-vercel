...
const research = new Map([
  ...
  ["300002", {
    ...
    financial: { latestRevenueGrowth: 45, latestProfitGrowth: 120, profitCagr3Y: 40, marginTrend: 4, roe: 20, roeTrend: 3, ocfToProfit: 120 },
    governanceScore: 8,
    valuation: {
      rankingEligible: true,
      strategicProbabilityWeighted: { marketCapYi: 600 },
      futureScenarios: { neutral: { marketCapYi: 600 } },
      marketCapacity: {
        valid: true,
        samYi: 800,
        capacityRevenueYi: 160,
        industryProfitPoolYi: 35,
        scenarios: { neutral: { companySharePct: 15, netMarginPct: 20, peerPe: 25 } }
      },
      invalidReasons: []
    }
  }],
...
assert.ok(result.futureFiveXCandidates.some(item => item.code === "300002"), "通过五倍数学验证、现金流与治理校验的样本应进入正式候选");
...
assert.equal(qualified.fiveXPassed, true, "五倍正式候选必须通过独立数学验证与否决校验");
assert.equal(qualified.fiveXMath.targetMcapYi, 600, "五倍模型必须反推当前市值五倍的目标市值");
