...
assert.ok(unknown.valuation.invalidReasons.includes("行业完全无法识别，缺少适配估值框架"));

const legalOnlyIndustry = buildCompanyResearchSnapshot(
  company("300998", "法定行业可识别样本", "半导体材料", { peTtm: 24 }),
  financial({ consensusProfitYi: 10, consensusRevenueYi: 80 }),
  {},
  context
);
assert.equal(legalOnlyIndustry.industry.confidence, "low");
assert.equal(legalOnlyIndustry.valuation.valid, true, "只有法定行业证据时仍应输出保守估值");
assert.equal(legalOnlyIndustry.valuation.rankingEligible, true, "行业可匹配且有未来盈利证据时应允许参与排名");
assert.equal(legalOnlyIndustry.valuation.confidence, "low", "行业证据不足必须降低情景置信度");
assert.ok(legalOnlyIndustry.valuation.warnings.some(item => item.includes("不因该项单独取消估值")));

const industryConflict = buildCompanyResearchSnapshot(
  company("300997", "行业冲突样本", "化学制品", { coreBusiness: "AI服务器与算力设备", peTtm: 24 }),
  financial({ consensusProfitYi: 10, consensusRevenueYi: 80 }),
  {},
  context
);
assert.equal(industryConflict.industry.conflict, true);
assert.equal(industryConflict.valuation.valid, true, "行业冲突仍应展示情景估值");
assert.equal(industryConflict.valuation.rankingEligible, false, "行业冲突未核验前不得进入推荐排名");
assert.ok(industryConflict.valuation.warnings.some(item => item.includes("不进入推荐排名")));
