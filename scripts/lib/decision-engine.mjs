...
function valuationDistance(holding, research) {
  ...
  const neutral = actionEligible ? numeric(research?.valuation?.probabilityWeighted?.targetPrice ?? scenarios.neutral?.targetPrice) : null;
  ...
  return {
    ...
    strategicNeutralTargetPrice: actionEligible ? numeric(research?.valuation?.strategicProbabilityWeighted?.targetPrice ?? strategic.neutral?.targetPrice) : null,
