...
function percentageRangeText(minValue, maxValue) {
  const min = toNumber(minValue);
  const max = toNumber(maxValue);
  if (min === null && max === null) return "同比变动待披露";
  if (min !== null && max !== null) return `${min.toFixed(1)}% 至 ${max.toFixed(1)}%`;
  return `${(min ?? max).toFixed(1)}%`;
}

function normalizedTushareEventDate(value) {
  const raw = String(value || "").replace(/[^0-9]/g, "");
  return raw.length >= 8 ? `${raw.slice(0, 4)}-${raw.slice(4, 6)}-${raw.slice(6, 8)}` : dateOnlyChina();
}

// These are structured earnings disclosures available under the current Tushare financial-data entitlement.
// They complement, rather than replace, original announcement sources from exchanges/CNINFO.
async function fetchTushareRecentEarningsEvents(days = 21) {
  if (!TUSHARE_TOKEN) {
    return { events: [], byCode: new Map(), forecastCount: 0, expressCount: 0, datesChecked: 0, source: "Tushare业绩事件未配置" };
  }
  const forecasts = [];
  const expresses = [];
  const errors = [];
  const dates = Array.from({ length: days }, (_, index) => tushareDateOffset(index));
  for (const annDate of dates) {
    const [forecastRows, expressRows] = await Promise.all([
      fetchTushare(
        "forecast",
        { ann_date: annDate },
        ...
        errors.push(`forecast ${annDate}: ${error.message}`);
        return [];
      }),
      fetchTushare(
        "express",
        { ann_date: annDate },
        "ts_code,ann_date,end_date,revenue,operate_profit,total_profit,n_income,total_assets,total_hldr_eqy_exc_min_int,diluted_eps,diluted_roe,yoy_revenue,yoy_op,yoy_tp,yoy_net_profit,yoy_eps,yoy_roe,growth_assets,yoy_equity,bps,perf_summary,is_audit"
      ).catch(error => {
        errors.push(`express ${annDate}: ${error.message}`);
        return [];
      })
    ]);
    forecasts.push(...forecastRows);
    expresses.push(...expressRows);
  }

  const byCode = new Map();
  const events = [];
  const appendLatest = (code, kind, row) => {
    if (!code) return;
    const existing = byCode.get(code) || {};
    const currentDate = String(row.ann_date || "");
    const existingDate = String(existing[`${kind}Date`] || "").replace(/-/g, "");
    if (currentDate >= existingDate) byCode.set(code, { ...existing, [`${kind}Date`]: normalizedTushareEventDate(row.ann_date), [`${kind}Row`]: row });
  };
  for (const row of forecasts) {
    const code = tushareCodeToAShareCode(row.ts_code);
    appendLatest(code, "forecast", row);
    events.push({
      date: normalizedTushareEventDate(row.ann_date),
      source: "Tushare业绩预告",
      name: "",
      code,
      priority: "财报事件",
      title: `${row.end_date || "报告期"}业绩预告`,
      type: "财报/业绩预告",
      importance: "高：直接影响盈利预期与估值",
      url: "https://tushare.pro/document/2?doc_id=45",
      facts: [
        `报告期：${row.end_date || "待披露"}`,
        `归母净利润同比变动：${percentageRangeText(row.p_change_min, row.p_change_max)}`,
        row.change_reason || row.summary ? `原因：${row.change_reason || row.summary}` : "业绩变动原因待读原文"
      ],
      analystRead: "业绩预告只给出区间，必须同时判断低基数、一次性损益、毛利率和现金流质量。",
      action: "高增长不等于追高；先看预告是否由主营收入、产品价格或订单兑现驱动。",
      trigger: "预告超预期且后续成交、板块资金与公司指引共同确认。",
      fail: "低基数、非经常损益或利润率恶化导致的表面高增。"
    });
  }
  for (const row of expresses) {
    const code = tushareCodeToAShareCode(row.ts_code);
    appendLatest(code, "express", row);
    events.push({
      date: normalizedTushareEventDate(row.ann_date),
      source: "Tushare业绩快报",
      name: "",
      code,
      priority: "财报事件",
      title: `${row.end_date || "报告期"}业绩快报`,
      type: "财报/业绩快报",
      importance: "高：已披露核心经营指标",
      url: "https://tushare.pro/document/2?doc_id=46",
      facts: [
        `报告期：${row.end_date || "待披露"}`,
        `营收同比：${percentageRangeText(row.yoy_revenue, row.yoy_revenue)}`,
        `归母净利润同比：${percentageRangeText(row.yoy_net_profit, row.yoy_net_profit)}`,
        row.perf_summary ? `摘要：${row.perf_summary}` : "快报摘要待读原文"
      ],
      analystRead: "业绩快报优先级高于预告，但仍需等待正式报告核验现金流、毛利率和应收变化。",
      action: "若增长来自主营且利润率改善，可提高跟踪优先级；不以单一快报直接满仓。",
      trigger: "快报增速、订单/行业景气与股价资金承接一致。",
      fail: "营收增长但利润率、现金流或应收账款同步恶化。"
    });
  }
  const uniqueEvents = [...new Map(events
    .filter(item => item.code)
    .map(item => [`${item.code}|${item.date}|${item.title}`, item]))
    .values()]
  ...
  return {
    ...
    source: errors.length ? `Tushare业绩预告/快报（部分接口失败${errors.length}次）` : "Tushare业绩预告(forecast)+业绩快报(express)",
...
async function fetchHoldingHardEvents(previous = {}, dailyCandidates = [], fiveXIdeas = [], valueIdeas = []) {
  ...
  // CNINFO requires an organisation id for some records, but SSE/SZSE/Eastmoney do not.
  // Do not silently drop tracked stocks merely because their CNINFO mapping is absent.
  const stocks = Array.from(stockMap.values()).slice(0, 80);
...
function valueIndustryProfile(row, override) {
  ...
  const detected = detectIndustryFamily({
    legalIndustry: row.industry || "",
    coreBusiness: businessText
  });
  const scores = { S: 25, A: 20, B: 13, C: 8 };
  const tier = detected.strategyTier === "C" && override?.tier
    ? override.tier
    : detected.strategyTier;
  return {
    tier,
    score: scores[tier] ?? 8,
    label: override?.industry || detected.level2 || row.industry || "未分行业",
    chain: override?.chain || detected.strategyTierReason || "产业链位置待核验",
    source: `${detected.matchedBy || "行业分类"} / ${detected.strategyEvidence || "待核验"}`,
    familyId: detected.familyId,
    evidence: detected.strategyEvidence
  };
}
...
function buildMacroMap(indices, globalMarkets = [], internals = {}) {
  ...
  const broadTrendBroken = shTrendScore <= -3;
  const growthTrendBroken = growthTrendScore <= -3;
  const structuralPullback = internalStrong && growthTrendScore >= 2 && (cyb < -2 || kc < -3);
  const bearEvidenceCount = [broadTrendBroken, growthTrendBroken, internalWeak, globalTechWeak].filter(Boolean).length;
  const marketRegime =
    broadStrong && growthStrong && !globalTechWeak ? "全面牛市观察"
      : growthStrong && !broadWeak && !globalTechWeak ? "结构性牛市"
      : structuralPullback ? "结构性牛市回撤"
      : bearEvidenceCount >= 2 && !internalStrong && (growthTrendBroken || broadTrendBroken) ? "熊市预警"
    ...
    marketRegime: {
      ...
      summary: marketRegime === "全面牛市观察"
        ...
        : marketRegime === "结构性牛市"
          ...
          : marketRegime === "结构性牛市回撤"
            ? "成长指数出现单日急跌，但中期趋势与全A宽度尚未同步破坏，属于结构性行情中的高波动回撤；先降追涨冲动，等主线承接确认，不直接按熊市清仓。"
          : marketRegime === "熊市预警"
      ...
      positionGuide: marketRegime === "全面牛市观察"
        ...
        : marketRegime === "结构性牛市"
          ...
          : marketRegime === "结构性牛市回撤"
            ? "维持中性仓位，停止追高；保留有业绩和中期趋势的核心仓，弱势反抽减仓，等待主线重新放量。"
          : marketRegime === "熊市预警"
...
async function main() {
  ...
  const tushareEarningsResult = await fetchTushareRecentEarningsEvents().catch(error => {
    console.warn(`Tushare earnings-event fallback: ${error.message}`);
    return { events: [], byCode: new Map(), forecastCount: 0, expressCount: 0, datesChecked: 0, source: `Tushare业绩事件失败：${error.message}`, errors: [error.message] };
  });
  for (const [code, data] of tushareEarningsResult.byCode) {
    const express = data.expressRow || {};
    const forecast = data.forecastRow || {};
    valueFinancialResult.byCode.set(code, {
      ...(valueFinancialResult.byCode.get(code) || {}),
      latestEarningsExpressDate: data.expressDate || null,
      latestEarningsExpressRevenueGrowth: toNumber(express.yoy_revenue),
      latestEarningsExpressProfitGrowth: toNumber(express.yoy_net_profit),
      latestEarningsForecastDate: data.forecastDate || null,
      latestEarningsForecastProfitGrowthLow: toNumber(forecast.p_change_min),
      latestEarningsForecastProfitGrowthHigh: toNumber(forecast.p_change_max)
    });
  }
